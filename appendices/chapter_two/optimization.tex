...
\begin{proof}
	...
	\begin{align*}
		-\tilde X^\top \rbr{b - \tilde X \tilde u}_+
		 & \in \rho \cdot \partial \norm{\tilde u}_2.
	\end{align*}
	Noting that every vector in \( \partial \norm{\tilde u}_2 \) has norm at most
	\( 1 \), we deduce
	\begin{align*}
		\norm{\tilde X^\top \rbr{b - \tilde X \tilde u}_+}_2
		 & \leq \rho                                        \\
		\implies
		\norm{\rbr{b - \tilde X \tilde u}_+}_2
		 & \leq \frac{\rho}{\sigma_{\text{min}}(\tilde X)}  \\
		\iff \norm{\rbr{\max\cbr{- \tilde X w, 0} - \tilde X \tilde u}_+}_2
		 & \leq \frac{\rho}{\sigma_{\text{min}}(\tilde X)}.
		...
		\norm{\rbr{\tilde X \tilde u}_-}_2
		 & \leq \frac{\rho}{\sigma_{\text{min}}(\tilde X)}  \\
		\norm{\rbr{\tilde X (w + \tilde u)}_-}_2
		 & \leq \frac{\rho}{\sigma_{\text{min}}(\tilde X)},
	\end{align*}
	Recalling \( \tilde v = w + \tilde u \) and summing gives the first result.

	If \( \rho > 0 \), then it is easy to observe
	(i.e. by arguing via contradiction) that \( \tilde u \) must
	have smaller norm than any \( u' \) in a feasible decomposition \( (v', u') \).
	Thus, it must also have smaller norm than \( \bar u \) from the SOCP cone
	decomposition:
	\[ \norm{\tilde u}_2 \leq \norm{\bar u}_2. \]
	Since the proof of \cref{prop:socp-decomp} relies on controlling
	only the norm of \( \bar u \), the conclusion of that theorem also applies
	to \( (\tilde v, \tilde u) \).
	...
	\begin{equation}\label{eq:cd-approx-constr}
		\textbf{CD-A}: \, \min_{w} g(u, \rho) : =
		\half
		\|
		(b - \tilde X u)_+
		\|_2^2
		+ \rho P(u) \quad \text{s.t.} \norm{u}_2 \leq \norm{\bar u}_2,
	\end{equation}
	where \( \bar u \) is the norm of the minimum-norm (with respect to \( u \)
	only) solution to the cone decomposition problem.
	This is a minimization problem with compact constraint set;
	since \( g \) is continuous in both \( u \) and \( \rho \),
	...
	Let \( \tilde u_k \) be the sequence of minimizers associated with \( \rho_k \).
	Since \( \tilde u_k \) is bounded, it has at least one convergent subsequence.
	Let \( \tilde u_0 \) be the associated limit point.
	Since \( g \) is continuous in \( u \) and \( \rho \), we find
	\[ g(u_0, 0) = \lim_k g(\tilde u_k, \rho_k) = \lim_k g^*(\rho_k) = 0, \]
	which shows that \( \tilde u_0 \) is a feasible decomposition.
...
\end{restatable}
\begin{proof}
	First-order optimality conditions for \( \tilde u \) imply
	\begin{align*}
		-\tilde X^\top \rbr{b - \tilde X \tilde u}_+
		 & \in \rho \cdot \partial \norm{\tilde u}_2.
	\end{align*}
	Let \( \calJ = \cbr{i \in [n] : b_i - \abr{\tilde x_i, \tilde u} > 0 } \)
	...
	\begin{align*}
		-\tilde X^\top B\rbr{b - \tilde X \tilde u}
		 & \in \rho \cdot \partial \norm{\tilde u}_2  \\
		\iff
		-(B \tilde X^\top)\rbr{b - \tilde X \tilde u}
		 & \in \rho \cdot \partial \norm{\tilde u}_2.
	\end{align*}
	Noting that every vector in \( \partial \norm{\tilde u}_2 \) has norm at most
	\( 1 \), we deduce
	\begin{align*}
		\norm{(B \tilde X)^\top \rbr{b - \tilde X \tilde u}}_2
		 & \leq \rho                                                \\
		\iff
		\norm{\tilde X_{\calJ}^\top \rbr{b - \tilde X \tilde u}_{\calJ}}_2
		 & \leq \rho                                                \\
		\implies
		\norm{\rbr{b - \tilde X \tilde u}_{\calJ}}_2
...
\[
	\min_{w} \; \half \norm{\sum_{D_i \in \tilde \calD} D_i X w_i - y}_2^2 + \lambda \sum_{D_i \in \tilde \calD} \norm{w_i}_2.
\]
The Hessian of the smooth component of this problem is \( \nabla^2 f(w) = M^\top M \), where \( M \) is the ``expanded'' data matrix \( M = \sbr{D_1 X \; D_2 X \; \ldots D_{|\tilde \calD|} X} \).
...
Thus, although the condition number of \( \nabla^2 f(w) \) is not well-defined, it is possible to reduce the maximum expansion entailed by the Hessian via column normalization.
Observing \( \nabla f(w) = \nabla^2 f(w) w - \tilde N^\top y \), we may expect conditioning of the gradient computation to improve.
Finally, since the C-GReLU is a linear model, transforming the weights as \( w_i' = H^{-1} w_i \) after optimization can be used to project the model back into the original data space, ensuring that data normalization has no effects outside of optimization.
