...
\begin{restatable}{lemma}{miFormulation}\label{lemma:mi-reformulation}
	...
	\begin{equation}\label{convex-forms:eq:mi-reformulation}
		\begin{aligned}
			\min_{W_1, w_2} \; & \calL\Big(\sum_{i = 1}^m \rbr{X W^{*(1)}_{\cdot, i}}_+ W^{*(2)}_{i}, y\Big) + \lambda \sum_{i = 1}^m \norm{W^{*(1)}_{\cdot, i}}_2 \\
...
\begin{proof}
	...
	\begin{align*}
		...
		= h(W'_{1}, w_2'),
	\end{align*}
	implying that the loss \( \calL(\sum_{i=1}^m \rbr{X W^{*(1)}_{\cdot, i} }_+ W^{*(2)}_{i}, y) \) is invariant to ``scale-shift'' re-parameterizations of this form.
	...
	\begin{align*}
		\calL\rbr{\sum_{i = 1}^m \rbr{X W^{*(1)}_{\cdot, i} }_+ W^{*(2)}_{i}, y } + \frac{\lambda}{2} \sum_{i = 1}^m \norm{W^{*(1)}_{\cdot, i}}_2 + \abs{W^{*(2)}_{i}}^2
		 & \geq \calL\rbr{ \sum_{i = 1}^m \rbr{X W^{*(1)}_{\cdot, i} }_+ W^{*(2)}_{i}, y } + \lambda \sum_{i = 1}^m \norm{W^{*(1)}_{\cdot, i}}\abs{W^{*(2)}_{i}}.
	\end{align*}
    ...
    As this rescaling does not affect \( h_{W_1, w_2} \), it must be that any
	...
	\end{align}
	The right-hand side of this equation is invariant to scale re-parameterizations of the form \( \bar{W}^{(1)}_{\cdot, i} = \beta W^{*(1)}_{\cdot, i}*, \bar{W}^{(2)}_{i} = W^{*(2)}_{i}* / \beta \) for \( \beta > 0 \).
	...
	Using the mapping
	\[ \rbr{W'_{1i}, \bar{W}^{(2)}_{i}} = \rbr{\frac{W^{*(1)}_{\cdot, i}*}{\sqrt{\norm{W^{*(1)}_{\cdot, i}*}_2}}, w^*_{2i} \sqrt{\norm{W^{*(1)}_{\cdot, i}*}_2}}, \]
...
\begin{proof}
	...
	\begin{equation*}
		\cbr{\rbr{v_j^*, w_j^*}}_{D_j \in \tilde \calD} = \cbr{ \sum_{i \in T(j)} W^{*(1)}_{\cdot, i}* \mathds{1}(W^{*(2)}_{i}* = 1), \sum_{i \in T(j)} W^{*(1)}_{\cdot, i}* \mathds{1}(W^{*(2)}_{i}* = -1) }_{D_j \in \tilde \calD}
	\end{equation*}
...
\begin{proof}
	...
	\begin{equation}
		\min_{W_1,w_2} \mathcal{L}\Big(\sum_{D_i \in \tilde \calD } D_i X W^{*(1)}_{\cdot, i} W^{*(2)}_{i}, y\Big) + \frac{\lambda}{2} \sum_{z_i \in \tilde \calZ} \norm{W^{*(1)}_{\cdot, i}}_2^2 + W^{*(2)}_{i}2,
	\end{equation}
	...
	\begin{align*}
		\mathcal{L}\Big(\sum_{D_i \in \tilde \calD } D_i X W^{*(1)}_{\cdot, i} W^{*(2)}_{i}, y\Big) + \frac{\lambda}{2} \sum_{z_i \in \tilde \calZ} \norm{W^{*(1)}_{\cdot, i}}_2^2 + W^{*(2)}_{i}2
		 & \geq \mathcal{L}\Big(\sum_{D_i \in \tilde \calD } D_i X W^{*(1)}_{\cdot, i} W^{*(2)}_{i}, y\Big) + \lambda \sum_{z_i \in \tilde \calZ} \norm{W^{*(1)}_{\cdot, i}}_2 \abs{W^{*(2)}_{i}}
	\end{align*}
	For any choice of parameters \( W^{*(1)}_{\cdot, i}, W^{*(2)}_{i} \), equality in this expression is achieved with the rescaling
	\[
		\bar{W}^{(1)}_{\cdot, i} = W^{*(1)}_{\cdot, i} * \beta_i, \quad \quad \bar{W}^{(2)}_{i} = W^{*(2)}_{i} / \beta_i,
...
\begin{equation}\label{convex-forms:eq:vector-non-convex-relu-mlp}
	\min_{W_1, W_2} \mathcal{L}\Big(\sum_{i=1}^m (XW^{*(1)}_{\cdot, i})_+ W_{2i}^\top, Y\Big) + \frac{\lambda}{2}\sum_{i=1}^m \norm{W^{*(1)}_{\cdot, i}}_2^2 + \norm{W_{2i}}_1^2,
\end{equation}
...
\begin{lemma}
	...
	\begin{equation}\label{convex-forms:eq:vector-mi-reformulation}
		\begin{aligned}
			\min_{\{W_{1}^k, w_{2}^k\}_{k=1}^C}  \; & \mathcal{L}\Big(\sum_{i=1}^m (XW^{*(1)}_{\cdot, i})_+ W_{2i}^\top, Y\Big) + \lambda \sum_{i = 1}^{m}  \norm{W^{*(1)}_{\cdot, i}}_2 \\
...
\begin{proof}
	...
	\begin{equation*}
		p^* = \min_{W_1, W_2} \mathcal{L}\Big(\sum_{i=1}^m (XW^{*(1)}_{\cdot, i})_+ W_{2i}^\top, Y\Big) + \frac{\lambda}{2}\sum_{i=1}^m \norm{W^{*(1)}_{\cdot, i}}_2^2 + \norm{W_{2i}}_1^2 = \min_{W_1, W_2} \mathcal{L}\Big(\sum_{i=1}^m (XW^{*(1)}_{\cdot, i})_+ W_{2i}^\top, Y\Big) + \lambda\sum_{i=1}^m \norm{W^{*(1)}_{\cdot, i}}_2\norm{W_{2i}}_1
	\end{equation*}
	Then, this is clearly equivalent to
	\begin{equation*}
		\begin{aligned}
			\min_{W_{1}, W_2}  \; & \mathcal{L}\Big(\sum_{i=1}^m (XW^{*(1)}_{\cdot, i})_+ W_{2i}^\top, Y\Big) + \lambda \sum_{i = 1}^{m}  \norm{W^{*(1)}_{\cdot, i}}_2 \\
			                      & \text{\emph{s.t.}} \;\|W_{2i}\|_1 = 1~\forall i \in [m]
		\end{aligned}
	\end{equation*}
\end{proof}
...
\begin{proof}
	...
	\end{align}
	Now, we seek to find the other direction, i.e. show \(p^* \geq d^*\) and show a mapping. Let \(\rbr{W^{*(1)}_{\cdot, i}*, W_{2i}^*}\) be a solution to \eqref{convex-forms:eq:vector-mi-reformulation}. Defining, as in \cref{thm:duality-free},
	\[
		T(j) = \cbr{ i \in [m] : \text{diag}\rbr{X W^{*(1)}_{\cdot, i}* > 0} = D_j }.
	\]
	Recalling
	...
	by assumption, we define a valid candidate solution as
	\begin{equation*}
		\cbr{\cbr{\rbr{v_j^{k*}, w_j^{k*}}}_{k \in [C]}}_{D_j \in \tilde \calD} = \cbr{ \cbr{\sum_{i \in T(j)} W^{*(1)}_{\cdot, i}*W^{*(2)}_{i}{k*} \mathds{1}(W_{2i}^{k*} \geq 0)}_{k \in [C]}, \cbr{-\sum_{i \in T(j)} W^{*(1)}_{\cdot, i}*W^{*(2)}_{i}{k*} \mathds{1}(W_{2i}^{k*} < 0)}_{k \in [C]} }_{D_j \in \tilde \calD}
	\end{equation*}
	Then, by the same co-linearity arguments as in \cref{thm:duality-free}, we have
	\begin{align*}
		p^* & =  \mathcal{L}\Big(\sum_{i=1}^m (XW^{*(1)}_{\cdot, i}*)_+ W_{2i}^{*\top}, Y\Big) + \lambda \sum_{i = 1}^{m}  \norm{W^{*(1)}_{\cdot, i}*}_2                                                                                   \\
		    & = \mathcal{L}\Big(\sum_{i=1}^m \sum_{k=1}^C (XW^{*(1)}_{\cdot, i}*)_+ W_{2i}^{k*}e_k^\top, Y\Big) + \lambda \sum_{i = 1}^{m}  \norm{W^{*(1)}_{\cdot, i}*}_2 \sum_{k=1}^C |W_{2i}^{k*}|                                       \\
		    & = \mathcal{L}\Big(\sum_{i=1}^m \sum_{k=1}^C (XW^{*(1)}_{\cdot, i}*)_+ W_{2i}^{k*}e_k^\top, Y\Big) + \lambda \sum_{i = 1}^{m} \sum_{k=1}^C \norm{W^{*(1)}_{\cdot, i}*}_2  |W_{2i}^{k*}|                                       \\
