...
\begin{proof}
	...
	\]
	If \( W^{(1)}_{\cdot, i} \) and \( W_{1j} \) are associated with the same pattern
	\( D_i \) and \( \text{sign}(W^{(2)}_{i}) = \text{sign}(w_{2j}) \),
	then \citet{mishkin2022convex} prove \( W^{(1)}_{\cdot, i} \) and \( W_{1j} \)
	are collinear, i.e. \( W^{(1)}_{\cdot, i} = \alpha W_{1j} \) for some \( \alpha > 0 \).
	They also prove that the model obtained by merging these collinear neurons
	as
	\[
		W'_{1i} = \frac{W^{(1)}_{\cdot, i}|W^{(2)}_{i}| + W_{1j}|w_{2j}|}{\norm{W^{(1)}_{\cdot, i}|W^{(2)}_{i}| + W_{1j}|w_{2j}|}_2^{1/2}},
		\quad \quad
		w'_{2i} = \text{sign}(W^{(2)}_{i})
		\norm{W^{(1)}_{\cdot, i}|W^{(2)}_{i}| + W_{1j}|w_{2j}|}_2^{1/2},
	\]
...
\begin{proof}
	...
	fact that \( \dim \Span(\cbr{(X W^{*(1)}_{\cdot, i})_+}_i) \leq n \).
	This completes the proof.
\end{proof}
...
\begin{proof}
	...
	\( (W^{(1)'}_{\cdot, i}, W^{(2)'}_{i}) \) and \( (W_{1j}', w_{2j}') \) such that
	\( W^{(1)'}_{\cdot, i} \) and \( W_{1j}' \) are collinear, \( \text{sign}(W^{(2)'}_{i}) =
	\text{sign}(w_{2j}') \), and
	\[
		W^{(1)}_{\cdot, i} = \frac{W^{(1)'}_{\cdot, i} W^{(2)'}_{i} + W_{1j}' w_{2j}'}{\norm{W^{(1)'}_{\cdot, i} W^{(2)'}_{i} + W_{1j}' w_{2j}'}_2^{1/2}},
		\quad \quad
		W^{(2)}_{i} = \text{sign}(W^{(2)}_{i})\norm{W^{(1)'}_{\cdot, i} W^{(2)'}_{i} + W_{1j}' w_{2j}'}_2^{1/2},
	\]
	and \( \norm{W^{(1)'}_{\cdot, i}}_2 = \norm{W^{(2)'}_{i}}_2 \),
	\( \norm{W_{1j}'}_2 = \norm{w_{2j}'}_2 \).
	...
	\begin{align*}
		\norm{W^{(1)}_{\cdot, i}}_2^4 + \norm{W^{(2)}_{i}}_2^4
		 & = 2 \norm{W^{(1)'}_{\cdot, i} W^{(2)'}_{i} + W_{1j}' w_{2j}'}_2^{2}                                   \\
		 & = 2 \norm{W^{(1)'}_{\cdot, i} W^{(2)'}_{i}}_2^2 + 2 \norm{W_{1j}' w_{2j}'}_2^2
		+ 4 \abr{W^{(1)'}_{\cdot, i} W^{(2)'}_{i}, W_{1j}' w_{2j}'}                                              \\
		 & > 2 \norm{W^{(1)'}_{\cdot, i} W^{(2)'}_{i}}_2^2 + 2 \norm{W_{1j}' w_{2j}'}_2^2                        \\
		 & = \norm{W^{(1)'}_{\cdot, i}}_2^4 + \norm{W^{(2)'}_{i}}_2^4 + \norm{W_{1j}'}_2^4 + \norm{w_{2j}'}_2^4,
	\end{align*}
