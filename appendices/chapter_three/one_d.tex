...
\begin{proposition}\label{prop:1d-to-lasso}
	...
	\begin{equation}\label{eq:1d-relu}
		\begin{aligned}
			\min_{w_1, W^{(2)}, b} \half \norm{\sum_{i=1}^m (Z w_{1i} + b_{1i})_+ W^{(2)}_{i} + b_2 - y}_2^2
			+ \frac{\lambda}{2} \norm{w_1}^2_2 + \norm{W^{(2)}}_2^2,
		\end{aligned}
	\end{equation}
...
\begin{proof}
	...
	\begin{equation}
		\begin{aligned}
			\min_{w_1, W^{(2)}, b_1, b_2} & \half \norm{\sum_{i=1}^m (Z w_{1i} + b_{1i})_+ W^{(2)}_{i} + b_2 - y}_2^2 + \lambda \norm{W^{(2)}}_1 \\
			                          & \hspace{5em} \text{ s.t. } w_{1i} \in \cbr{+1 , -1}.
		\end{aligned}
	\end{equation}
	Now we introducing the constraint \( a = \sum_{i=1}^m (Z w_{1i} + b_{1i})_+ W^{(2)}_{i} + b_2 \)
	so that we may compute the dual problem,
	\begin{align*}
		p^* & = \min_{w_1, W^{(2)}, b_1, b_2, a}
		\half \norm{a - y}_2^2 + \lambda \norm{w_1}^2_2 + \norm{W^{(2)}}_2^2                                                                                                    \\
		    & \hspace{5em} \text{ s.t. } a = \sum_{i=1}^m (Z w_{1i} + b_{1i})_+ W^{(2)}_{i} + b_2, \, w_{1i} \in \cbr{+1, -1}
		\intertext{
			Forming the Lagrangian and infimizing over the primal parameters,
		}
		d(\eta)
		    & = \min_{w_1, W^{(2)}, b_1, b_2, a} \half \norm{a - y}_2^2 + \abr{\eta, a}
		+ \abr{\eta, \sum_{i=1}^m (Z w_{1i} + b_{1i})_+ W^{(2)}_{i} + b_2} + \lambda \norm{W^{(2)}}_1 + \mathds{1}_{\cbr{+1, -1}^m}(w_{1})                                          \\
		    & = \min_{w_1, W^{(2)}, b_1, b_2} \half \norm{\eta - y}_2^2 + \half \norm{y}^2_2
		+ \abr{\eta, \sum_{i=1}^m (Z w_{1i} + b_{1i})_+ W^{(2)}_{i} + b_2} + \lambda \norm{W^{(2)}}_1 + \mathds{1}_{\cbr{+1, -1}^m}(w_{1})                                          \\
		    & = \min_{w_1, W^{(2)}, b_1} \half \norm{\eta - y}_2^2 + \half \norm{y}^2_2
		+ \abr{\eta, \sum_{i=1}^m (Z w_{1i} + b_{1i})_+ W^{(2)}_{i}} + \lambda \norm{W^{(2)}}_1 + \mathds{1}_{\cbr{+1, -1}^m}(w_{1}) - \mathds{1}_{\{0\}}(\eta^\top \mathds{1})   \\
		    & = \min_{w_1, W^{(2)}, b_1} \half \norm{\eta - y}_2^2 + \half \norm{y}^2_2
		+ \abr{W^{(2)}, (Z w_{1}^\top + b_{1i} \mathds{1})_+ \eta} + \lambda \norm{W^{(2)}}_1 + \mathds{1}_{\cbr{+1, -1}^m}(w_{1}) - \mathds{1}_{\{0\}}(\eta^\top \mathds{1}) \\
		    & = \min_{w_1, b_1} \half \norm{\eta - y}_2^2 + \half \norm{y}^2_2
