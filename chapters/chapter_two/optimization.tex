...
\begin{restatable}{theorem}{gatedComplexity}\label{thm:gated-complexity}
	Let \( \rbr{W_{1}^*, w_2^*} \) be the minimum-norm global minimizer of the NC-GReLU problem with gates \( \calG \).
	Then, we can compute an \( \epsilon \)-optimal point \( \rbr{W_{1\epsilon}, w_{2\epsilon}} \) in iterations
	\begin{equation*}
		\begin{aligned}
			T & \leq \big(2 \epsilon^{-1} \lambda_{\text{max}}\rbr{M^\top M}\sum_{D_i \in \tilde \calD} \norm{W_{1i}^* w_{2i}^*}_2^2\big)^{1/2}.
		\end{aligned}
	\end{equation*}
\end{restatable}
