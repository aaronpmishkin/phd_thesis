...
Throughout this section, we will focus on training two-layer ReLU networks
using convex optimization.
We consider the standard training problem with weight decay regularization,
\begin{equation*}
    h_{W_1, w_2}(X) = \sum_{i=1}^m \rbr{X W^{(1)}_{\cdot, i}}_+ W^{(2)}_{i},
\end{equation*}
...
\begin{restatable}{theorem}{convexDualityFree}\label{thm:duality-free}
    Suppose \( \theta^* = \rbr{W^{*(1)}, W^{*(2)}} \) and \( \rbr{v^*, w^*} \)
...
%\begin{equation*}
%    \rbr{\hat v^*, \hat w^*}  =  \bigg\{ \sum_{i \in T(j)}  W^{(1)}_{\cdot, i}^* \mathds{1}(W^{(2)}_{i}^* = 1),    \sum_{i \in T(j)}    W^{(1)}_{\cdot, i}^* \mathds{1}(W^{(2)}_{i}^* = -1) \bigg\}_{j = 1}^m
%\end{equation*}
...
\[
    h_{W_1, w_2}(X) = \sum_{g_i \in \calG} \phi_{g}(X, W^{(1)}_{\cdot, i}) W^{(2)}_{i},
\]
...
\begin{restatable}{theorem}{unconstrainedEquivalence}\label{thm:unconstrained-equivalence}
    ...
    \begin{equation}\label{convex-forms:eq:gated-relu-mlp}
         \min_{W_1, w_2} \calL\Big(\sum_{g_i \in \tilde \calG} 
