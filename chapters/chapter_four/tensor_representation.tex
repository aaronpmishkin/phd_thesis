...
\begin{equation}\label{eq:l-activation-patterns}
    \begin{aligned}
        ...
        : \theta^{(l-1)} \in \Theta^{(l-1)}, u \in \R^{d_{l}}
        \big\}.
    \end{aligned}
\end{equation}
...
operator on \( \Xk \).
To do so, we define a tensor reduction as follows.
\begin{definition}[Tensor Reduction]
    Given two tensors
    \( \Xll \in \R^{n \times p_1 \times \cdots \times p_l \times d_1} \)
    and
    \( \Tll \in \R^{d_1 \times p_1 \times \cdots \times p_l \times d_{l+1}} \),
...
Unrolling \cref{prop:layer-elimination} from \( l = L \) to \( l = 1 \)
...
\begin{restatable}{theorem}{tensorEquivalence}\label{thm:tensor-equivalence}
    ...
    tensor parameterization \( \Tk \in \calG^{(k)} \) satisfying
    \[
...
\begin{corollary}\label{cor:non-convex-tensor-program}
    Training an \( L \)-layer ReLU model,
    \begin{equation}
        \min_{\theta = (W^{(1)}, \ldots, \Wk)} \calL\rbr{f_{\theta}(X), y},
    \end{equation}
    is equivalent to a convex tensor program
    with non-convex decomposition constraints,
...
has \( d_1 p_1 c \) parameters, which is exponential when \( X \)
is full-rank).
