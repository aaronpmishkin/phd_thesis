...
operator on \( \Xk \).
To do so, we define our tensor reduction as follows.
...
\( l \), such as \( U^{(l)} \in \R^{d_1 \times p_1 \times \cdots \times p_{l-1}} \)
in which case it is given by the following reduction over
matrix-vector products:
\[
    \Xll \odot U^{(l)} = \sum_{j_{l}} \cdots \sum_{j_{1}}
    \Xll_{j_1, \ldots, j_{l}} U^{(l)}_{j_1, \ldots, j_{l}, i_{l+1}}.
\]
With the tensor reduction operator now in hand, we can define the tensor cones
...
and our tensor cones reduce directly to the two-layer setting.

Now we have the notation to establish a tensorized version of our neural
network \( f_\theta \).
Key to this representation is a recursive constraint set encoding the network
structure in tensor form.
Define \( \calG^{(1)} = \R^{d_1 \times d_2} \) and let \( \calG^{(l)} \)
...
\begin{equation}\label{eq:rank-tensor-decomp}
    ...
    \end{aligned}
\end{equation}
Now we prove that each \( L \)-layer ReLU neural network can be
represented by a tensor of order \( L+1 \).
\begin{restatable}[Layer Elimination]{lemma}{layerElimination}\label{lemma:layer-elimination}
    Let \( T^{(1)} = W^{(1)} \).
    There exists \( T^{(l)} \in \calG^{(l)} \) such that
    ...
    \begin{equation}\label{eq:layer-to-tensorize}
        ...
        \rbr{\sum_{i_l=1}^{d_l} \rbr{X^{(l)} \odot T^{(l)}_{i_l}}_+
            W^{(l+1)}_{i_l}}_+,
    \end{equation}
    if and only if there exists \( T^{(l+1)} \in \calG^{(l+1)} \) satisfying
    \[
