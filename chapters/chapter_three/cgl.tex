...
Our first result combines KKT conditions with uniqueness of \( \hat y(\lambda) \)
to characterize the solution set for fixed \( \lambda > 0 \).
...
\input{algorithms/cgl_pruning}

\cref{alg:pruning-solutions} gives a procedure which, starting from
any optimal solution \( w \), computes a optimal model with minimal
...
In such situations, we propose to the solve the following elastic-net-type problem
...
\begin{definition}[LICQ]
	\( w \! \in \! \solfn(\lambda, y) \) satisfies linear independence
	CQ if \( \cbr{[K]_j : j \! \in \! \calB(w)} \) are linearly independent.
\end{definition}
