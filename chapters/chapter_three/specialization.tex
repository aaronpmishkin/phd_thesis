...
The mapping from convex to non-convex parameterization
transforms the polytope of solutions into a curved manifold.

Choosing a sub-sampled set of patterns
\( \tilde \calD \subset \calD_Z \) corresponds to finding a stationary
point of the non-convex training problem \citep{wang2021hidden}.
...
\( (W_1^*, w_2^*) \) are optimal weights for the ReLU model, then
\( m^* \) is at most the dimension of
\( \Span \cbr{(X W^*_{1i})_+}_i \)
...
\begin{restatable}{proposition}{oneNeuron}\label{prop:one-neuron}
	...
	is not open nor is the model fit \( f_{W_1, w_2}(Z) \) continuous in
	\( \lambda \).
\end{restatable}
