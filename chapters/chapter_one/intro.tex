...
In comparison, stationary points can be computed for smooth, convex functions
in \( \tilde{O}(\epsilon^{-1/2}) \) iterations using acceleration
...
\begin{figure}[t]
    ...
    \caption{Illustration of critical points for a non-convex function.
        ...
        Local minima like \( \tilde{x}_1 \), local maxima like \( \tilde{x}_2 \)
        and saddle-points (not shown) make both optimization and analysis
        of non-convex functions challenging.
    }%
...
reformulations.
Convex reformulations re-write the non-convex training problems for neural
networks as convex programs by lifting them into higher dimensional spaces
\citep{pilanci2020convexnn}.
Crucially, the resulting problems are finite-dimensional.
This makes convex reformulations more practical to solve than convexifications
of neural networks based on infinite-width relaxations
\citep{bengio2006convex}.
It also means that they allow for more realistic analyses:
while infinite-width neural networks provide powerful insights into
generalization behavior \citep{bach2017breaking}, convex reformulations
characterize only (and, in fact, all) finite-width networks
\citep{wang2022hidden}.
...
As such, advances in training and theory for two-layer networks can
...
contained in an affine subspace.
...
To the best of our knowledge, this is the first pruning algorithm for neural
networks which is guaranteed to preserve global optimality.
...
For sufficiently wide (although still finite) networks, these low-rank
...
activation patterns.
This equivalence yields a fully unconstrained convex reformulation for
training deep ReLU models.
...
\cref{chapter:conclusion} concludes the thesis and introduces several open
problems which we believe are important to advancing the theory of convex
reformulations.
All proofs are postponed to the corresponding section of the appendix.
