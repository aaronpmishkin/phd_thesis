...
Attempts to develop convex versions of neural networks have a long history that
originates in large-width analyses of shallow networks.
\citet{neal1996priors} were the first to show that infinite-width two-layer
neural networks with appropriate priors on the network weights converge to
convex Gaussian process models.
...
However, these models rely on a Bayesian approach that views the network
...
The innovation of the NTK is to extend the Gaussian process
dynamics to describe the behavior of the network during training;
this is achieved by showing that the effects of training vanish in the
infinite-width limit and the network remains at the kernel specified by the
...
learning.
Since the effect of training vanishes as the network becomes arbitrarily wide,
...
optimal networks where number of units is chosen automatically by a sparsity
inducing norm.
...
minima vanish if the network is sufficiently wide, while \citet{wang2021hidden}
