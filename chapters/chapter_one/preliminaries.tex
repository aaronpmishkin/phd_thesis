...
In this case, the recursive definition can be unrolled to give an explicit
prediction function,
...
objective function and ensures that \( F_\lambda \) is coercive and the
sub-level sets are compact.

\begin{figure}[t]
    \centering
    \input{figures/chapter_one/activation_pattern.tex}
...
In the following, we assume that \( \theta \) parameterizes a two-layer,
scalar output ReLU network as described in \cref{eq:two-layer-scalar-output}.
For such networks, convex reformulations re-write
Problem~\ref{eq:regularized-erm} as a convex program by enumerating the
activations a single neuron in the hidden layer can take on for fixed \( X \)
as follows:
...
\end{equation}
If \( u \in \calK_i \), then \( u \) is said to match, or be consistent with,
\( D_i \), meaning \( D_i X u = \rbr{X u}_+ \).
...
\begin{figure}[t]
    \centering
    \input{figures/chapter_one/convex_reformulation.tex}
...
\citet{pilanci2020convexnn} prove that this program is equivalent to
training a two-layer ReLU network are equivalent in the following sense:
if \( \tilde \calD = \calD_X \) and \( m \geq m^* \) for some \( m^* \leq n + 1 \),
then the two problems have the same optimal value and every solution to the
convex program can be mapped to a solution of the non-convex training problem
and vice-versa.
In particular, given a solution \( \rbr{v^*, u^*} \), optimal weights
for the ReLU problem are given by
\begin{equation}\label{eq:convex-to-relu}
    \begin{aligned}
        W^{(1)}_{\cdot, i} 
        & = v_i^*/ \sqrt{\norm{v_i^*}_2}, \quad W^{(2)}_{i} = \sqrt{\norm{v_i^*}_2}
        \\
        W_{1j}             
        & = u_i^*/ \sqrt{\norm{u_i^*}_2}, \quad w_{2j} = -\sqrt{\norm{u_i^*}_2},
    \end{aligned}
\end{equation}
...
are low rank.
In such special settings, \citet{pilanci2020convexnn} prove that training two-layer
ReLU networks is fully polynomial in the problem parameters.
This implies global optimization is polynomial time in fixed dimension as long
as the network width exceeds \( m^* \).
In fact, \( m^* \) is exactly the critical threshold for two-layer ReLU
networks to be in P (instead of NP) since \citet{froese2023hard} provide a lower
bound showing global optimization is NP-Hard for fixed \( d \) as long as
\( m \) is taken sufficiently small.
In the following chapter, we take a different approach and provide refined
conditions on \( \tilde{\calD} \) for Problem~\ref{eq:convex-relu-mlp} to be
equivalent to the non-convex problem.
