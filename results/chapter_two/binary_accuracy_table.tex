...
\begin{table}[t]
	\centering
	\caption{Test accuracies for our convex solvers, random forests (RF), SVMs with a linear kernel (Linear)
		and SVMs with an RBF kernel (RBF) for binary classification on 18 UCI datasets.
		C-GReLU and C-ReLU both obtain the best test accuracy on 9 datasets,
		while the most competitive baseline is best on just 4.
	}%
	\label{table:binary-uci-accuracies}
	\vspace{0.1in}
	\begin{small}
